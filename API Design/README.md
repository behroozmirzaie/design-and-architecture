...
* Tue Feb 01 2022 23:57:12 GMT-0800 (PST)
...
<p dir="rtl">
در معماری REST، سرور REST به سادگی دسترسی به منابع را فراهم می کند و مشتری REST به منابع دسترسی پیدا میکند. در اینجا هر منبع با URI ها/ شناسه های جهانی شناسایی می شود. REST از نمایش های مختلفی برای نشان دادن resource مانند متن، JSON و XML استفاده می کند. امروزه JSON محبوب ترین فرمتی است که در وب سرویس ها استفاده می شود.</p>
...
1. سرور می بیند که یک قیمت تغییر کرده است و بلافاصله یک پیام برای هر مشتری ارسال می کند.
2. مشتری پیام قیمت جدید را دریافت می کند.
...
1. client یک بازه برای گرفتن تغییرات  تنظیم می کند(interval polling)
2. پس از شروع interval pollign بعدی، client اتصال سوکت به سرور ایجاد می کند
3. سرور درخواست باز کردن سوکت جدید را دریافت می کند
4. هنگامی که اتصال با سرور برقرار می شود، client درخواست اطلاعات قیمت جدید را به سرور ارسال می کند
5. سرور درخواست اطلاعات قیمت گذاری جدید را دریافت می کند و پاسخ را با داده های جدید (در صورت وجود) ارسال می کند.
6. client داده های قیمت گذاری جدید را دریافت می کند
7. مشتری سوکت را می بندد
8. سرور سوکت بسته می شود
...
* Use Plural Noun  از اسم جمع استفاده کنید.
    * به عنوان مثال برای منابع کاربران از کلمه users استفاده میکنیم نه user
* Avoid using spaces از فاصله استفاده نکنیم.
    * هنگام استفاده از نام resource طولانی، از خط (_) یا خط فاصله (-) استفاده کنید، به عنوان مثال، از authorized_users به جای authorized users استفاده کنید.(به space دقت بفرمایید :D)
* Use lowercase letters از حروف کوچک استفاده کنیم.
    * اگرچه URI به حروف بزرگ و کوچک حساس نیست، اما تمرین خوبی است که url را فقط با حروف کوچک نگه دارید.
* Maintain Backward Compatibility سازگاری با ورژن های گذشته  را حفظ کنید
    * از آنجایی که وب سرویس یک سرویس عمومی است، یک URI زمانی که عمومی شد باید همیشه در دسترس باشد. در صورت به روز رسانی URI، با استفاده از کد وضعیت HTTP 300 آدرس های قدیمی را به URI جدید هدایت کنید.
* Use HTTP Verb
...
* در زیر نکات مهمی وجود دارد که باید در هنگام طراحی representation یک resource در یک سرویس وب RESTful در نظر گرفته شود.
    1. Understandability - قابل درک بودن.
        * هم سرور و هم کلاینت باید بتوانند فرمت نمایش resource  را درک کرده و از آن استفاده کنند
    2. Completeness کامل بودن
        * قالب باید بتواند یک resource را به طور کامل نشان دهد.به عنوان مثال، یک resource می تواند حاوی resource دیگری باشد.قالب باید بتواند ساختارهای ساده و همچنین پیچیده resource را نشان دهد.
    3. Linkability قابلیت پیوند داشته باشد.
        * یک resource می تواند به resource دیگری پیوند داشته باشد، یک قالب باید بتواند چنین شرایطی را مدیریت کند.
...
* 201: CREATED، زمانی که یک منبع با موفقیت با استفاده از درخواست POST یا PUT ایجاد می شود. با استفاده از سرصفحه مکان، پیوند را به یک منبع جدید ایجاد شده برگردانید.
* 304: NOT MODIFIED برای کاهش استفاده از پهنای باند شبکه در صورت درخواست های GET conditional استفاده می شود.بدنه پاسخ باید خالی باشد. Header  ها باید تاریخ، مکان و… داشته باشند.
* 400: BAD REQUEST, بیان می کند که ورودی نامعتبر ارائه شده است به عنوان مثال. خطای اعتبارسنجی، داده های از دست رفته ,...
* 401: FORBIDDEN بیان می کند که کاربر به method  مورد استفاده دسترسی ندارد
* 404: NOT FOUND بیان می کند که method در دسترس نیست.
* 409: CONFLICT وضعیت conflict را هنگام اجرای متد بیان می کند، به عنوان مثال، اضافه کردن یک ورودی تکراری.
* 500:  INTERNAL SERVER ERROR بیان می کند که سرور در حین اجرای متد exception ایجاد کرده است.
...
* فقط XML می تواند استفاده شود، JSON و سایر فرمت های سبک پشتیبانی نمی شوند.
* SOAP بر اساس قرارداد است، بنابراین یک tight coupling بین client application  و سرور وجود دارد.
* SOAP کند است زیرا Payload برای یک پیام رشته ای ساده بزرگ است چون از فرمت XML استفاده می کند.
* هر زمان که در قرارداد سمت سرور تغییری ایجاد شود کلاینت ها باید مجدد ایجاد شوند
* به راحتی در مرورگر تست نمی شود
...
* Swapping asynchronous messages over a message oriented middle-ware (MOM)
    * مبادله پیام های asynchronous بر روی یک message oriented middle-ware
...
1. Confidentiality قابل اطمینان بودن.
2. Authentication
3. Network Security
...
* HTTP Version
* Response Header: اطلاعات اضافی که به صورت key-value در header قرار میگیرد.
* Response Body
...
* Public: منابعی که با pulic مشخص بشن توسط هر مولفه میانی بین client و server میتونند ذخیره بشن.
* Private: منابعی که با Private مشخص بشن فقط توسط client قابل cache شدن هستند.
* No cache: منبع خاصی را نمیتوان کش کرد.
...
* REST - یک architecture برای اینکه چگونه یک پیغام HTTP ارسال کنیم.
* OData (v4) - نوع خاصی از پیاده سازی REST هستش و در حقیقت محتوای پیغام را با فرمت های مختلف مشخص می کنه. و در نظر داشته باشیم که ODataV4 قوانین rest را پیروی می کنه.
